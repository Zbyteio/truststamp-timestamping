...
export default function SetupContainer() {
	...
	const [githubConnectionStatusStyle, setGithubConnectionStatusStyle] =
		useState({});
	const [verifiedGithubAccessToken, setVerifiedGithubAccessToken] =
		useState("");
		...
		if (status === "authenticated" && session?.user?.email) {
			...
			const checkIfDatabaseIsEmpty = async () => {
				const response = await fetch(
					`/api/database/checkDatabaseEmpty?email=${encodeURIComponent(email)}`
				);
	...
	const loadCredentials = async (submittedPassword: string) => {
		const savedGithubToken = await fetchCredential(
			"github",
			"token",
			submittedPassword
		);
		const savedGithubId = await fetchCredential(
			"github",
			"id",
			submittedPassword
		);
		const savedAccessKey = await fetchCredential(
			"aws",
			"accessKey",
			submittedPassword
		);
		const savedSecretKey = await fetchCredential(
			"aws",
			"secretKey",
			submittedPassword
		);
		const savedBucket = await fetchCredential(
			"database",
			"bucket",
			submittedPassword
		);
		...
		const savedTimestampFrequency = await fetchCredential(
			"timestampFrequency",
			"value",
			submittedPassword
		);
		...
		<div className={styles.mainPart}>
			...
			<div style={{ paddingBlockStart: "40px" }}>
				...
				<div style={{ maxWidth: "600px", margin: "0 auto" }}>
					<div style={{ marginBottom: "30px" }}>
						<h2 className={styles.headerPart}>Github Connection</h2>
						{githubId && (
						...
						{githubConnectionStatus && (
							<p style={githubConnectionStatusStyle}>
								{githubConnectionStatus}
							</p>
						)}
					</div>
					<div style={{ marginBottom: "30px" }}>
						<h2 className={styles.headerPart}>Select Database Service</h2>
						...
						{connectionStatus && (
							<p style={connectionStatusStyle}>{connectionStatus}</p>
						)}
					</div>
					...
					<div style={{ marginBottom: "30px" }}>
						...
						<select
							...
							className={styles.timeStampSelector}
						>
							<option value="" disabled>
								Select Frequency
							</option>
					...
					{!isDatabaseEmpty && (
						<button
							onClick={handleReset}
							className={styles.databaseEmptyButton}
						>
							{resetLoading ? (
								<div className={styles.centerize}>
									<Circles color="#FFF" height={20} width={20} />
