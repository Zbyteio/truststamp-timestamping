...
export default function TrackFeatureContainer() {
	...
	const [fetchedFilesContent, setFetchedFilesContent] = useState<
		...
		if (status === "authenticated" && session?.user?.email) {
			...
			const initialize = async () => {
				try {
					...
					if (!response.ok) {
						throw new Error(
							`Failed to fetch decrypted password: ${response.statusText}`
						);
					}
	...
	useEffect(() => {
		...
		(async () => {
			...
			const { accessKey, secretKey, bucket } = await fetchAwsCredentials(
				password
			);

			// excluding zipped github files
			const featureFiles = feature.files.filter(
				(f: any) => !/[\dT]+Z\.zip$/.test(f.originalName)
			);

			const contents = await Promise.all(
				featureFiles.map((file: any) => {
			...
			const fetchedFiles = featureFiles.map((file: any, i: number) => {
				const ext =
					file.originalName.includes(".") && file.originalName.split(".").pop();
				const type = imageExts.includes(ext) ? `image/${ext}` : "";
				return new File([Buffer.from(contents[i])], file.originalName, {
					type,
				});
			});
			...
			setFetchedFilesContent(
				contents.map((contents: string, i: number) => ({
					file: fetchedFiles[i],
					contents,
				}))
			);
	...
	const fetchFeature = async (featureId: number) => {
		...
		if (!res.ok)
			return console.warn(`Could not fetch feature with id ${featureId}.`);
	...
	const fetchOrganizations = async (email: string, password: string) => {
		...
		try {
			const response = await fetch(
				`/api/github/organizations?email=${email}&password=${password}`
			);
	...
	const fetchRepositories = async (org: string, setState = true) => {
		...
		try {
			const response = await fetch(
				`/api/github/repositories?email=${email}&password=${password}&org=${org}`
			);
			const data = await response.json();
	...
	const fetchBranches = async (repository: string, setState = true) => {
		...
		try {
			const response = await fetch(
				`/api/github/branches?repository=${repository}&email=${email}&password=${password}`
			);
			const data = await response.json();
	...
	const fetchFileStructure = async (
		...
		try {
			const response = await fetch(
				`/api/github/contents?email=${email}&password=${password}&repository=${repository}&branch=${branch}&path=${path}`
			);
			const data = await response.json();
			if (path) {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const zipBlob = await zipSelectedFilesAndFolders(
				selectedFiles,
				selectedFolders
			);
			...
			const fileNames = [
				...
				{
					original: zipBlob,
					originalName: `${selectedRepository}/${selectedBranch}-selected-${timestamp}.zip`,
					random: zipFileName,
				},
			];
			...
			const fileDataWithHashes = await Promise.all(
				fileNames.map(async (file) => {
					...
					const transactionHash = await hashifyAndPublish(
						file.original,
						file.random
					);
					return {
	...
	const hashifyAndPublish = async (file: File | Blob, fileName: string) => {
		try {
			...
			const response = await fetch(
				`/api/wallet/writeToChain?publicKey=${encodeURIComponent(
					publicKey
			...
			console.log(
				`File ${fileName} published on chain successfully with transaction hash: ${result.transactionHash}`
			);
		...
		return files.map((file) => (
			<div key={file.path} className={styles.fileItem}>
				<input
					...
					checked={
						file.type === "dir"
							? selectedFolders.includes(file.path)
				...
				<span
					onClick={
						file.type === "dir" ? () => handleFolderClick(file.path) : undefined
					}
				>
	...
	const renderFilePreview = (file: File, isRemote: boolean = false) => {
		...
		if (isImage) {
			if (isRemote) {
				const base64 = fetchedFilesContent.find(
					(f) => f.file === file
				)?.contents;
				const ext = {
					"": "png",
			...
			>
				...
				{!isRemote && (
					<button
						...
						onClick={() => handleRemoveFile(uploadedFiles.indexOf(file))}
					>
						Remove
					</button>
				)}
	...
	const fetchFileContent = async (path: string) => {
		const response = await fetch(
			`/api/github/content?email=${email}&password=${password}&repository=${selectedRepository}&branch=${selectedBranch}&path=${path}`
		);
		if (!response.ok) {
	...
	const zipSelectedFilesAndFolders = async (
		...
		const addFileToZip = async (path: string, zipFolder: JSZip) => {
			const response = await fetch(
				`/api/github/file?email=${email}&password=${password}&repository=${selectedRepository}&branch=${selectedBranch}&path=${path}`
			);
			if (!response.ok) {
				throw new Error(`Failed to fetch file content: ${response.statusText}`);
			}
		...
		const addFolderToZip = async (
			...
			const response = await fetch(
				`/api/github/folder?email=${email}&password=${password}&repository=${selectedRepository}&branch=${selectedBranch}&path=${folderPath}`
			);
			...
			for (let item of folderContent) {
				if (item.type === "dir" && zipFolder) {
					...
					if (zipFolder) await addFileToZip(item.path, zipFolder);
				}
			}
		};
	...
	const fetchCredential = async (
		...
		const response = await fetch(
			`/api/database/credentials?email=${email}&service=${service}&key=${key}&password=${password}`
		);
		const data = await response.json();
		return data.value;
	};
	...
	const uploadFileToDatabase = async (file: File | Blob, fileName: string) => {
		...
		const savedDatabaseUrl = await fetchCredential(
			"firebase",
			"databaseUrl",
			password
		);
		const savedServiceAccount = await fetchCredential(
			"firebase",
			"serviceAccount",
			password
		);
		...
		if (savedAccessKey && savedSecretKey) {
			...
			if (!awsResponse.ok) {
				throw new Error(
					`Failed to upload file to AWS: ${awsResponse.statusText}`
				);
			}
		}
		...
		if (savedDatabaseUrl && savedServiceAccount) {
			...
			if (!firebaseResponse.ok) {
				throw new Error(
					`Failed to upload file to Firebase: ${firebaseResponse.statusText}`
				);
			}
		}
	};
				...
				<div className={styles.formContainer}>
					...
					<div>
						...
						<div className={styles.crumbValues}>
							...
							{crumb > 1 && selectedRepository && (
								<div>Repo: {selectedRepository}</div>
							)}
							{crumb > 2 && selectedBranch && (
						...
						{crumb === 0 && (
							<div
								className={`${styles.selectOrganizations} ${styles.listContainer}`}
							>
								...
								{orgLoading ? (
									...
									<select
										defaultValue={initialValues?.org || selectedOrg}
						...
						{crumb === 1 && (
							<div
								className={`${styles.selectRepositories} ${styles.listContainer}`}
							>
								{repoLoading ? (
									<div style={{ display: "flex" }}>
										<Circles color="#00BFFF" height={40} width={40} />
									...
									<>
										...
										<select
											defaultValue={initialValues?.repo || selectedRepository}
									...
									<h2>
										No repositories found for organization &quot;{selectedOrg}
										&quot;.
									</h2>
								)}
							</div>
						...
						{crumb === 2 && (
							<div
								className={`${styles.selectBranches} ${styles.listContainer}`}
							>
								{branchLoading ? (
									<div style={{ display: "flex" }}>
										<Circles color="#00BFFF" height={40} width={40} />
									...
									<>
										...
										<select
											defaultValue={initialValues?.branch || selectedBranch}
									...
									<h2>
										No branches found for repository &quot;{selectedRepository}
										&quot;.
									</h2>
								)}
							</div>
